...
public class CropImageView1 extends ImageView {
    ...
    protected FloatDrawable1 mFloatDrawable;
    ...
    private void init(Context context) {
        ...
        mFloatDrawable = new FloatDrawable1(context);
    }
    ...
    // }
    // }
    // 它的逻辑是：在move中滑动就已经绘画好了截取框，然后再放开的时候进行截取框的移动。但宽高不变。
    protected void checkBounds() {
        int newLeft = mDrawableFloat.left;
        int newTop = mDrawableFloat.top;
        ...
        int buttom = mDrawableFloat.bottom;

        boolean isChange = false;

        if (mDrawableFloat.left < mDrawableDst.left) {// 左边超了
            newLeft = mDrawableDst.left;
            isChange = true;
            left = mDrawableDst.left;

        }

        if (mDrawableFloat.top < mDrawableDst.top) {
            newTop = mDrawableDst.top;
            isChange = true;
            top = mDrawableDst.top;
        }

        if (mDrawableFloat.right > mDrawableDst.right) {

            newLeft = mDrawableDst.right - mDrawableFloat.width();
            isChange = true;
            right = mDrawableDst.right;
        }

        if (mDrawableFloat.bottom > mDrawableDst.bottom) {
            newTop = mDrawableDst.bottom - mDrawableFloat.height();

            buttom = mDrawableDst.bottom;
            isChange = true;
        }
        if (currentEdge != EDGE_MOVE_IN) {
            mDrawableFloat.set(left, top, right, buttom);
        } else {
            mDrawableFloat.offsetTo(newLeft, newTop);
        }
